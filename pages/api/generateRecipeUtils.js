...
export function generatePrompt(requestBody) {
    // Destructure all
    const { 
        messageHistory,
        selectedDiet,
    ...
    // If message history, return a repeat prompt
    if (messageHistory > 0) {
        return `Generate ${recipeCount} more unique recipes that satisfy the original requirements defined.
    ...
    // If selected diet and ingredients, return prompt that ...
    if (selectedDiet && selectedIngredients) {
    ...
    // If selected recipes, return prompt that ...
    else if (selectedRecipes) {
