...
// Simulated user saved diets
// Delete once database integration is done:
...
const DietPage = () => {
   ...
   const handleSelectDiet = (selectedDiets) => {
      ...
      const isSelectedDietsMatchSaved = selectedDiets.every(selectedDiet =>
            userSavedDiets.includes(selectedDiet.name)
         ) && selectedDiets.length === userSavedDiets.length;
   ...
   useEffect(() => {
      ...
      if (useSavedDiets) {
         ...
         const savedDietObjects = diets.filter(diet => userSavedDiets.includes(diet.name));
         ...
         <Container className="mt-5">
            <Row className="justify-content-md-center">
               <Col md={12} className="text-center">
                  ...
                  <p className='text-muted'>Generate recipes based on a particular diet or dietary restriction. Select a diet or dietary restriction and we'll do the rest.</p>
               </Col>
            </Row>
            {generatePressed ? (
               <Container className='mb-4'>
                  <Row>
                     <Col md={6} className='text-end'>
                        <Button onClick={handleGenerateMoreClick} className='generate-recipe-btn' variant='success' size="lg" disabled={!recipes ? true : false}>
                           Generate New Recipes
                        </Button>
                     </Col>
                     <Col md={6}>
                        <Button onClick={handleStopGenerating} className='generate-recipe-btn' variant='danger' size="lg" disabled={!recipes ? true : false}>
               ...
               <Container>
                  <Row className="justify-content-center mb-4">
                     <Col className="flex-column align-items-center" md={9}>
                        <h5 className='diet-select-label'>Select a diet or dietary restriction</h5>
                        <Select
                           multi
                           options={diets}
                  ...
                  <Row className="justify-content-center mb-5">
                     <Col className="d-flex flex-column align-items-center">
                        <Button className='generate-recipe-btn' variant='success' size="lg" onClick={handleGenerateClick} disabled={selectedDiet == "" ? true : false}>
                           Generate Recipes
                        </Button>
                     </Col>
                  </Row>
