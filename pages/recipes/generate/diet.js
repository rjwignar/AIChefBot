...
import { cacheRecipes, fetchCachedRecipes } from "@/pages/api/sessionStorage";
...
const DietPage = () => {
  ...
  useEffect(() => {
    if (session) {
      ...
      // Check to see if cached recipes exist
      let recipes = fetchCachedRecipes();
      if (recipes.length) {
        console.log("Found cached recipes!");
        setRecipes(recipes);
        setGeneratePressed(true);
      }
      else console.log("No cached recipes found!");
    }
  }, []);
